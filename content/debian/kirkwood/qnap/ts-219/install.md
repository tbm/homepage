---
title: Installing Debian on QNAP TS-21x
nav: Installing Debian
description: Installation instructions for Debian on QNAP TS-21x
keywords: [Debian, QNAP, TS-210, TS-212, TS-219, TS-219P, TS-219P+, ARM, armel, installation]
---
...
<h1>Installing Debian on QNAP TS-21x devices</h1>

<h2>Overview</h2>

In a nutshell, the installation of Debian on your QNAP TS-21x works like
...
If you follow this procedure, Debian 6.0 (squeeze) will be installed to
...
In order to install Debian on a QNAP TS-21x device, you need the following:

<ul>

<li>A QNAP TS-210, TS-212, TS-219, TS-219P, TS-219P+ or TS-219P II.</li>
...
You have to make a backup of all the data stored on your TS-21x before
...
Once the command has completed, you can reboot your TS-21x:
...
device, the Debian installer will start.  Since the TS-21x does not have
...
Since the TS-21x does not have an LCD, the installer cannot tell you which
...
QFinder, DHCP will be used.  You can use the MAC address of your TS-21x to
...
<li>If DHCP is used but your DHCP server does not respond, the TS-21x will
use the fallback address `192.168.1.100`.  If you are unsure what the
address of your TS-21x is, unplug the Ethernet cable, start the machine
...
Please make sure that the TS-21x is on a local network to which only you
...
example with the actual IP address of your TS-21x) and login as user
...
guide</a>.  The installer knows about the TS-21x and at the end of the
...
Since the TS-21x boots from flash you don't have many limitations as to how
...
installed system via SSH.  The TS-21x will beep when you can connect via
...
use `apt-get` and other tools to install additional software.  The TS-21x
