---
title: Installing Debian on QNAP TS-109
nav: Installing Debian
description: Installation instructions for Debian on QNAP TS-109
keywords: [Debian, QNAP, TS-109, installation]
---
...
In a nutshell, the installation of Debian on your QNAP TS-109 works like
...
As of Debian 10 (buster), no installer is provided for the QNAP TS-109
...
In order to install Debian on a QNAP TS-109, you need the following:

<ul>

<li>A QNAP TS-109, TS-109 II, TS-109 Pro or TS-109 Pro II.</li>

<li>An internal SATA disk in the TS-109.</li>
...
You have to make a backup of all the data stored on your TS-109 before
...
your TS-109 depending on whether you have installed the full QNAP firmware.
The QNAP TS-109 only comes with a minimal Linux system in flash and
...
prepare your TS-109 for the installation of Debian if you have installed
...
TS-109 with telnet.  Use the QNAP web interface and follow these steps
...
Now that telnet is running, you can connect to your TS-109 with the
following command:
...
TS-109.  13131 is the default port used by the TS-109 for telnet.  Use
...
Turn on your TS-109 and wait a few minutes until it has started.  The
...
Disconnect your USB stick from your TS-109, connect it to your PC and to
...
mkdir /tmp/debian
cd /tmp/debian
busybox wget http://archive.debian.org/debian/dists/stretch/main/installer-armel/current/images/orion5x/network-console/qnap/ts-109/flash-debian
busybox wget http://archive.debian.org/debian/dists/stretch/main/installer-armel/current/images/orion5x/network-console/qnap/ts-109/model
busybox wget http://archive.debian.org/debian/dists/stretch/main/installer-armel/current/images/orion5x/network-console/qnap/ts-109/initrd
busybox wget http://archive.debian.org/debian/dists/stretch/main/installer-armel/current/images/orion5x/network-console/qnap/ts-109/kernel
</pre>
</div>
...
QFinder, DHCP will be used.  You can use the MAC address of your TS-109 to
...
<li>If DHCP is used but your DHCP server does not respond, the TS-109 will
use the fallback address <code>192.168.1.100</code>.  If you are unsure what the
address of your TS-109 is, unplug the Ethernet cable, start the machine
...
Please make sure that the TS-109 is on a local network to which only you
...
archive.  Since Debian 9 (stretch) is no longer distributed on Debian
...
use `apt` and other tools to install additional software.  The TS-109
